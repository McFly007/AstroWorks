...
struct Vector3 {
  ...
  Vector3 operator-() const { return this->operator*(-1); }
  ...
  float operator*(const Vector3& b) const {
    return m.x*b.m.x + m.y*b.m.y + m.z*b.m.z;
  }
  ...
  float& operator[](const unsigned int i) { return (&m.x)[i]; }
  const float& operator[](const unsigned int i) const { return (&m.x)[i]; }

  friend std::ostream &
  operator<<(std::ostream & os, const Vector3 & v) {
    os << "Vector3(" << v.m.x << ", " << v.m.y << ", " << v.m.z << ")";
    return os;
  }
};
